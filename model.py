...
class Model():
    ...
    def _build(self):
        ...
        # Modified original implementation to accommodate N coupled equations (2N in total)
        # Should be functional
        with tf.name_scope('diffusion_bridge'):
            v_paths, v_mu, v_sigma = self._diff_bridge()

        # ELBO loss not implemented yet for new model specification
        # with tf.name_scope('ELBO'):
        #     loss_estimate = ELBO(
        #         self.obs, v_paths, v_mu, v_sigma, self.pars, self.priors, self.p, self.dt, self.T)
        #     tf.summary.scalar('mean_loss', mean_loss)

        # Optimiser should be functional
        ...
        # To implement: report variables to tensorboard
    ...
    def _diff_bridge(self):
        ...
        '''
        x01 = tf.expand_dims(self.params['x0'].sample(self.p), axis = 2)
